...
namespace webmanager
{
    ...
    // Netzwerk-AGNOSTISCHE Basis des Webmanagers: HTTP(S)-Server-Handler, Login/Sessions/Rollen,
    // Nutzerverwaltung, Auslieferung der SPA, Websocket-Endpunkt inkl. Plugin-Dispatch, OTA,
    // Datei-Endpunkte, mDNS und SNTP. Enthaelt bewusst KEINEN WLAN-Code -- die konkrete
    // Netzwerk-Anbindung wird von abgeleiteten Klassen ueber die virtuellen Hooks beigesteuert:
    //   webmanager::M           -- WLAN (STA + AccessPoint-Fallback + Supervisor-State-Machine)
    //   webmanager_eth_only::M  -- reines Ethernet (Board bringt ETH bereits vor Begin() hoch)
    class aWebmanagerBase : public webmanager::iWebmanagerCallback
    {
    protected:
        // Wird im Konstruktor gesetzt. Wird von den statischen Trampolinen (ws_async_send,
        // SNTP-Callback) gebraucht, die keinen 'this'-Zeiger mitbekommen. Es gibt pro Geraet
        // genau eine Webmanager-Instanz (die konkrete Klasse haelt zusaetzlich ihr eigenes,
        // typisiertes GetSingleton()).
        static inline aWebmanagerBase *instance{nullptr};
        ...
        // Schuetzt ausschliesslich das Session-Array (Login/Logout/Validierung aus dem
        // HTTPD-Task heraus). Bewusst getrennt von der WLAN-Zustandssperre in webmanager::M --
        // die beiden schuetzen disjunkte Zustaende, und die Basis darf keinen WLAN-Zustand kennen.
        SemaphoreHandle_t sessions_semaphore{nullptr};
        ...
        // Generischer IP_EVENT-Handler. ETH-Ereignisse werden hier direkt behandelt (sie sind
        // netzwerk-agnostisch im Sinne von "kein WLAN-Zustand noetig"), alles andere (STA_GOT_IP,
        // STA_LOST_IP, ASSIGNED_IP_TO_CLIENT, ...) wird an OnIpEvent() der abgeleiteten Klasse
        // weitergereicht.
        ...
        {
            aWebmanagerBase *myself = aWebmanagerBase::instance;
        ...
        {
            xSemaphoreTake(sessions_semaphore, portMAX_DELAY);
            int slot = -1;
            ...
            xSemaphoreGive(sessions_semaphore);
        ...
        // "Template method" fuer den generischen Teil von Begin(). Wird von den Begin()-Methoden
        // der abgeleiteten Klassen aufgerufen, nachdem diese ihre eigenen Parameter verarbeitet
        // haben. Reihenfolge ist bewusst identisch zur fruheren monolithischen Begin()-Methode:
        //   Bootstrap-Admin -> netif/Event-Loop -> IP_EVENT -> BringUpNetwork() -> Hostname/mDNS
        //   -> SNTP -> StartNetworkStateMachine() -> Plugin-OnBegin
        esp_err_t BeginBase(const char *hostname, std::vector<iWebmanagerPlugin *> *plugins, bool init_netif_and_create_event_loop, const char *auth_username_param, const char *auth_password_param)
        {
            ...
            if (sessions_semaphore != nullptr)
            {
                ESP_LOGE(TAG, "webmanager already started. Exiting 'Begin'-method");
                return ESP_FAIL;
            }
            sessions_semaphore = xSemaphoreCreateBinary();
            xSemaphoreGive(sessions_semaphore);
            ...
            ESP_ERROR_CHECK(esp_event_handler_instance_register(IP_EVENT, ESP_EVENT_ANY_ID, [](void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
                                                                { static_cast<webmanager::aWebmanagerBase *>(arg)->ip_event_handler(event_base, event_id, event_data); }, this, nullptr));

            esp_err_t ret = BringUpNetwork();
            if (ret != ESP_OK)
            {
                ESP_LOGE(TAG, "BringUpNetwork() failed with %s. Exiting 'Begin'-method", esp_err_to_name(ret));
                return ret;
            }

            esp_netif_t *primary = GetPrimaryNetif();
            if (primary)
            {
                ESP_ERROR_CHECK(esp_netif_set_hostname(primary, hostname));
            }
            ...
            esp_sntp_set_time_sync_notification_cb([](struct timeval *tv)
                                                   { webmanager::aWebmanagerBase::instance->sntp_handler(); });
    ...
    public:
        virtual ~aWebmanagerBase() { delete[] http_buffer; }

        // Fuer Aufrufer, die nur wissen wollen "die eine, geraeteweite Webmanager-Instanz" (z.B.
        // Health-Monitoring in nodemaster.hh), unabhaengig davon, ob main.cc webmanager::M oder
        // webmanager_eth_only::M instanziiert hat. Liefert nullptr, solange keine konkrete Klasse
        // ihren Konstruktor durchlaufen hat.
        static aWebmanagerBase *GetActiveInstance() { return instance; }

        // --- Von der konkreten Klasse beizusteuernde Netzwerk-Anbindung ------------------------

        // Netif(s) anlegen/finden und den darunterliegenden Protokoll-Stack initialisieren -- aber
        // noch KEINE Verbindung aufbauen (das passiert in StartNetworkStateMachine()).
        virtual esp_err_t BringUpNetwork() = 0;

        // Verbindungsaufbau anstossen (WLAN: esp_wifi_start()/AP-Fallback/connect). Fuer bereits
        // von aussen hochgebrachte Netze (Ethernet) ein No-Op.
        virtual void StartNetworkStateMachine() {}

        // Das netif, dessen Hostname/IP-Adresse dieses Geraet nach aussen repraesentiert.
        virtual esp_netif_t *GetPrimaryNetif() = 0;

        // Einheitliche Netzwerk-Statusabfrage fuer Aufrufer, die nur einen aWebmanagerBase* haben
        // (z.B. Health-Monitoring): WLAN -> STA verbunden, Ethernet -> Link/netif up.
        virtual bool IsNetworkUp() const = 0;

        // Wird vom generischen ip_event_handler() fuer alle NICHT-ETH-IP-Ereignisse aufgerufen.
        virtual void OnIpEvent(int32_t event_id, void *event_data) {}

        // Periodischer Aufruf aus main.cc's Hauptschleife (WLAN: Reconnect-/AP-Fallback-
        // Zustandsmaschine voranschreiten lassen; Ethernet-only: nichts zu tun).
        virtual void Supervise() {}

        // Erlaubt der konkreten Klasse, eigene Websocket-Namespaces zu bedienen, bevor der
        // generische Plugin-Dispatch greift (webmanager::M nutzt das fuer den
        // wifimanager-Namespace). Signatur identisch zu iWebmanagerPlugin::ProvideWebsocketMessage.
        virtual eMessageReceiverResult ProvideWebsocketMessage(iWebmanagerCallback *callback, httpd_req_t *req, httpd_ws_frame_t *ws_pkt, uint16_t namespaceId, uint16_t messageTypeId, const uint8_t *frame, size_t frameLen)
        {
            return eMessageReceiverResult::NOT_FOR_ME;
        }

        // --- Generische oeffentliche API ------------------------------------------------------
        ...
        esp_err_t SendRawAsync(const uint8_t* data, size_t len) override
        {
            ...
            esp_err_t ret = httpd_queue_work(http_server, aWebmanagerBase::ws_async_send, a);
            if (ret != ESP_OK)
            {
        ...
        void RegisterHTTPDHandlers(httpd_handle_t httpd_handle)
        {
            httpd_uri_t files_get = {
                ...
                HTTP_GET,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_files_get(req); },
            ...
            httpd_uri_t files_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_files_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t files_delete = {
                ...
                HTTP_DELETE,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_files_delete(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t ota_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_ota_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t login_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_login_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t logout_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_logout_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t admin_users_get = {
                ...
                HTTP_GET,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_admin_users_get(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t admin_users_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_admin_users_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t admin_users_delete_post = {
                ...
                HTTP_POST,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_admin_users_delete_post(req); },
                this, false, false, nullptr};
            ...
            httpd_uri_t webmanager_ws = {
                ...
                HTTP_GET,
                [](httpd_req_t *req)
                { return static_cast<webmanager::aWebmanagerBase *>(req->user_ctx)->handle_webmanager_ws(req); }, this, true, false, nullptr};
            ESP_ERROR_CHECK(httpd_register_uri_handler(httpd_handle, &webmanager_ws));
            
            httpd_uri_t webmanager_get = {
                "/*", HTTP_GET,
                [](httpd_req_t *req)
                { return static_cast<aWebmanagerBase *>(req->user_ctx)->handle_webmanager_get(req); },
                this, false, false, nullptr};
        ...
        {
            esp_netif_get_hostname(GetPrimaryNetif(), &hostname);
            return hostname;
        }
        ...
        esp_ip4_addr_t GetIpAddress()
        {
            ...
            esp_netif_get_ip_info(GetPrimaryNetif(), &ip);
            return ip.ip;
        }
    };
}
